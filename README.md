...
- 样片保留原 `18 × 14` 地图、NPC 坐标、互动 ID、七天剧情和操作方式；除桥面可走范围与
  可见双排木板对齐外，只替换渲染层。
- 阶段 1B 已接入 `576 × 448` 高完成度原创场景底图与透明桥栏前景层；人物仍由游戏逻辑实时绘制，不使用烘焙角色。
- 河道、双排桥面、面包房占地、公告板、桌台与村口标记均已对齐碰撞格；桥右端用可见挡柱收口，不会出现“还有桥板却被隐形墙挡住”。
...
- 阶段 2B 已把面包房、双排木桥、春 4 药草棚、公告板、白布桌、村口节庆门和河岸
...
- 阶段 3A 已把玩家和五位具名村民统一到原生整数脚点：阿栀与四位村民具有两帧待机，
  对话中当前说话人会以两帧举手姿态回应；四方向转身、存档、剧情和操作保持原有逻辑。
- 新图集由既有原创角色素材确定性派生；加载失败时逐项回退既有图集或程序绘制，动画关闭时
  仍可完整移动、互动与通关。
...
- 阶段 3C 完成正式七天流程的环境演出：风铃、布条、水面与桥灯保持四帧整数像素节奏；
  春 4 新增雨滴落点与水洼涟漪，春 6 新增暖岸 / 冷河光界，春 7 新增错相连续风铃、
  两帧围观人群和低密度花瓣流。
- 春 6 三盏桥灯统一挂到桥栏侧，可见的上下两排桥面均连续可走；若玩家撞到桥外水面或桥边 NPC，
  画面会说明真实阻挡原因，不再产生“被灯笼卡住”的误解。
